 ...
 * @title PToken 抽象合约
 */
abstract contract PToken is IPToken, Exponential, TokenErrorReporter {
    /**
     * @notice 初始化市场
    ...
    function initialize(IRiskManager riskManager_,
        ...
        uint err = _setRiskManager(riskManager_);
        require(err == uint(Error.NO_ERROR), "setting riskManager failed");
     ...
     * @dev This is used by riskManager to more efficiently perform liquidity checks.
    ...
    /**
      * @notice Sets a new riskManager for the market
      * @dev Admin function to set a new riskManager
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function _setRiskManager(IRiskManager newRiskManager) public override returns (uint) {
        ...
        // Ensure invoke riskManager.isRiskManager() returns true
        require(newRiskManager.isRiskManager(), "marker method returned false");

        // Set market's riskManager to newRiskManager
        riskManager = newRiskManager;

        // Emit NewRiskManager(oldRiskManager, newRiskManager)
        emit NewRiskManager(oldRiskManager, newRiskManager);
